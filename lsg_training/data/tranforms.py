...
class TransformTrain(object):
    ...
    def __call__(self, x):
        return [self.normalize(self.weak(x)), self.normalize(self.weak(x))]

class TransformTrainStrong(object):
    def __init__(self,resize_size=256, crop_size=224, mean=imagenet_mean, std=imagenet_std):
    ...
    def __call__(self, x):
        return [self.normalize(self.strong(x)), self.normalize(self.strong(x))]
